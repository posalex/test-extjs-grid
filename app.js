Ext.onReady(function() {
    ...
    var workplacesThatNeedAComment = ["K1", "K2", "AE", "MK0", "MK1", "EI", "SP", "HU", "DE", "US", "HK", "DG"];
    var minCommentLength = 10;
    var workplacesWithPredefinedComments = {
        ...
        EI: ["Ensure equipment is operational", "Schedule maintenance checks", "Calibrate instruments", "Train new technicians", "Update safety protocols"],
        DG: ["Prepare presentation slides", "Organize team meeting", "Design new marketing materials", "Review client feedback", "Brainstorm product improvements"]
    };
...
The user enters a valid workplace, the start time and end time of a shift and a comment. The comment is optional or mandatory, depending on the workplace. \
...
1. For workplaces ' +   workplacesThatNeedAComment.join(', ') + ', a comment longer than ' + minCommentLength + ' characters must be entered, otherwise the field should be displayed as incorrect. \n\n\
2. In addition to the mandatory text entry for a personnel assignment at workplaces ' + 
Object.keys(workplacesWithPredefinedComments).join(', ')
...
'+ 
Object.keys(workplacesWithPredefinedComments).map(function(workplace) {
    return workplace + ':\n' + workplacesWithPredefinedComments[workplace].map(function(comment) {
        return '- "' + comment + '"';
    }).join('\n');
}).join('\n')
